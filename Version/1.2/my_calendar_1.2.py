                        ...
                        if component.name == "VEVENT" and component.get('dtstart'):
                            ...
                            if dtend_comp:
                                ...
                                if isinstance(dtend, datetime):
                                    # 자정(00:00:00)에 끝나는 일정의 경우 표기할 때는 그 전날까지로 표시해야 함
                                    end_date = (dtend - timedelta(seconds=1)).date()
                                else:
                                    # 종일 일정(date 타입)의 경우 dtend는 기간의 다음 날(exclusive)을 의미
